...
# Initialize previous frame and previous audio segment
prev_frame = None
...
while cap.isOpened():
    ...
    if prev_audio_segment is not None:
        # Convert previous audio segment to numpy array
        prev_samples = np.array(prev_audio_segment.get_array_of_samples())
        
        # Ensure both audio segments have the same length
        min_length = min(len(samples), len(prev_samples))
        samples = samples[:min_length]
        prev_samples = prev_samples[:min_length]
        
        # Compute the difference in amplitude
        amplitude_diff = np.abs(samples - prev_samples)
        
        # Print the sum of absolute differences in amplitude as a simple metric
        print(f'Frame {frame_number} Audio Amplitude Difference: {np.sum(amplitude_diff)}')

    # Update previous audio segment
